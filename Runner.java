...
//	    }
	    ...
	    //4.16
	    final int FIRST1 = 220; // first couple
	    final int FIRST2 = 284;
	    final int LAST1 = 66928; // 10th couple
	    final int LAST2 = 66992;
	    int count = 1;
	    for (int i = FIRST1; i<=LAST1; i++)
	    {
	    	for (int j=FIRST2; j<=LAST2; j++)
	    	{
		    	...
		    	for (int k=1; k<j; k++)
		    		if (j%k == 0)
		    			currentSum2 += k;
		    	if (currentSum1 == j && currentSum2 == i)
		    	{
		    		System.out.println(count+")"+i+" and "+j+" are mates");
		    		count++;
